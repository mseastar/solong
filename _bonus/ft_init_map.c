...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/04 00:10:11 by mseastar          #+#    #+#             */
...
static int	ft_pull_list(t_list *map, t_list *map_step, int fh1, int *i)
{
	while (map_step)
	{
		map_step->next = ft_lstnew_b(get_next_line(fh1));
		if (!map_step->next)
		{
			ft_lstclear_b(&map, &free);
			return (1);
		}
		else if (!(map_step->next->str))
		{
			ft_lstdelone_b(map_step->next, &free);
			map_step->next = NULL;
			break ;
		}
...
char	**ft_init_map_b(char *str)
{
	...
	if (ft_check_argv_b(str))
		return (NULL);
	i = 1;
	str = ft_strjoin_def_b("map/", str);
	fh1 = open(str, O_RDONLY);
	if (fh1 < 0)
		return (NULL);
	map = ft_lstnew_b(get_next_line(fh1));
	if (!map)
		return (NULL);
	map_step = map;
	if (ft_pull_list(map, map_step, fh1, &i))
		return (NULL);
	map_r = ft_map_create_b(map, i);
	ft_lstclear_b(&map, &free);
	close(fh1);
	free(str);
	return (map_r);
