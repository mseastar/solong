...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/28 13:52:28 by mseastar          #+#    #+#             */
/*   Updated: 2021/12/02 22:20:27 by mseastar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++count_y < 50)
	{
		...
		while (++count_x < 50)
		{
			if (count_x + (x * 50) >= 1200 || count_y + (y * 50) >= 1000)
				return ;
			my_mlx_pixel_put(vars->img_pixels, count_x + (x * 50), \
...
void	ft_draw_pixels(t_vars *vars)
{
	...
	y = -1;
	vars->img_pixels = ft_init_img(vars, 1200, 1000);
	if (!vars->img_pixels)
		return ;
	i = vars->fish->y - 1;
